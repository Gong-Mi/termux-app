...
public class TerminalRendererGLES implements GLSurfaceView.Renderer {
    ...
    public TerminalRendererGLES(int textSize, Typeface typeface) {
        updateFontInternal(textSize, typeface);
    }
    ...
    private void updateFontInternal(int textSize, Typeface typeface) {
        mTextSize = textSize;
        mTypeface = typeface;
        ...
        Log.d(TAG, "updateFontInternal: mFontWidth: " + mFontWidth + ", mFontLineSpacing: " + mFontLineSpacing);
    }
    ...
    private void generateMesh() {
        ...
        for (int row = 0; row < rows; row++) {
            int internalRow;
            try {
                internalRow = screen.externalToInternalRow(row);
            } catch (IllegalArgumentException e) {
                continue; // Skip rows that are temporarily out of bounds
            }
            TerminalRow line = screen.allocateFullLineIfNecessary(internalRow);
