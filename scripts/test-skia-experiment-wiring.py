...
class SkiaExperimentWiring(unittest.TestCase):
    ...
    def test_exact_head_ab_has_strict_oracle_and_preserves_artifacts(self):
        ...
        install = next(s for s in steps if s.get('id') == 'install-target')
        self.assertIn('adb install -r', install['run'])
        ab = next(s for s in steps if s['name'] == 'Verify Skia API A/B')
        self.assertLess(steps.index(install), steps.index(ab))
