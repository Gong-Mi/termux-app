...
/**
 * A circular buffer of {@link JavaJavaTerminalRow}:s which keeps notes about what is visible on a logical screen and the scroll
 * history.
 * <p>
 * See {@link #externalToInternalRow(int)} for how to map from logical screen rows to array indices.
 */
public final class JavaJavaTerminalBuffer {

    JavaJavaTerminalRow[] mLines;
     ...
     */
    public JavaJavaTerminalBuffer(int columns, int totalRows, int screenRows) {
        mColumns = columns;
        mTotalRows = totalRows;
        mScreenRows = screenRows;
        mLines = new JavaJavaTerminalRow[totalRows];
    ...
    public String getSelectedText(int selX1, int selY1, int selX2, int selY2, boolean joinBackLines, boolean joinFullLines) {
        ...
        for (int row = selY1; row <= selY2; row++) {
            ...
            JavaJavaTerminalRow lineObject = mLines[externalToInternalRow(row)];
    ...
    public void resize(int newColumns, int newRows, int newTotalRows, int[] cursor, long currentStyle, boolean altScreen) {
        ...
        if (newColumns == mColumns && newRows <= mTotalRows) {
            ...
            JavaJavaTerminalRow[] oldLines = mLines;
            mLines = new JavaJavaTerminalRow[newTotalRows];
            for (int i = 0; i < newTotalRows; i++)
                mLines[i] = new JavaJavaTerminalRow(newColumns, currentStyle);
            ...
            for (int externalOldRow = -oldActiveTranscriptRows; externalOldRow < oldScreenRows; externalOldRow++) {
                ...
                JavaJavaTerminalRow oldLine = oldLines[internalOldRow];
    ...
    private void blockCopyLinesDown(int srcInternal, int len) {
        ...
        JavaJavaTerminalRow lineToBeOverWritten = mLines[(srcInternal + start + 1) % totalRows];
    ...
    public void scrollDownOneLine(int topMargin, int bottomMargin, long style) {
        ...
        if (mLines[blankRow] == null) {
            mLines[blankRow] = new JavaJavaTerminalRow(mColumns, style);
        } else {
    ...
    public void blockCopy(int sx, int sy, int w, int h, int dx, int dy) {
        ...
        for (int y = 0; y < h; y++) {
            ...
            JavaJavaTerminalRow sourceRow = allocateFullLineIfNecessary(externalToInternalRow(sy + y2));
    ...
    public JavaJavaTerminalRow allocateFullLineIfNecessary(int row) {
        return (mLines[row] == null) ? (mLines[row] = new JavaJavaTerminalRow(mColumns, 0)) : mLines[row];
    }

    public void setChar(int column, int row, int codePoint, long style) {
        if (row  < 0 || row >= mScreenRows || column < 0 || column >= mColumns)
            throw new IllegalArgumentException("JavaTerminalBuffer.setChar(): row=" + row + ", column=" + column + ", mScreenRows=" + mScreenRows + ", mColumns=" + mColumns);
    ...
    public void setOrClearEffect(int bits, boolean setOrClear, boolean reverse, boolean rectangular, int leftMargin, int rightMargin, int top, int left,
        ...
        for (int y = top; y < bottom; y++) {
            JavaJavaTerminalRow line = mLines[externalToInternalRow(y)];
