...
fn test_expand_shows_content_below_viewport() {
    ...
    println!("   扩大后状态:");
    println!(
        "   active_transcript_rows = {} (之前：{})",
        engine.state.main_screen.active_transcript_rows, old_active
    );
    println!(
        "   first_row = {} (之前：{})",
        engine.state.main_screen.first_row, old_first
    );
    println!(
        "   rows = {} (之前：{})",
        engine.state.main_screen.rows, old_rows
    );
    println!("   cursor_y = {}", engine.state.cursor.y);
    ...
    for i in 1..=19 {
        ...
        if trimmed != expected_full {
            println!(
                "   ✗ 行 {} 应该是 '{}'",
                row,
                expected_full.chars().take(50).collect::<String>()
            );
            println!(
                "      但实际是 '{}'",
