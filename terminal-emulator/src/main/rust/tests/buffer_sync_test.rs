...
use termux_rust::terminal::style::{EFFECT_BOLD, STYLE_NORMAL, encode_style};
...
fn test_buffer_sync_basic_ascii() {
    ...
    let flat = engine
        ...
        .flat_buffer
        .as_ref()
        .expect("Flat buffer should exist");
...
fn test_buffer_sync_shared_memory() {
    ...
    use termux_rust::engine::shared_buffer::{SharedBufferPtr, SharedScreenBuffer};
    ...
    unsafe {
        assert!(
            (*ptr).version > 100,
            "Version should have incremented after sync"
        );
