...
fn test_ring_buffer_indexing() {
    ...
    println!(
        ...
        get_active_transcript_rows(&engine)
    );

    // 验证：应该能看到 Line 5-9
    assert!(
        row0_text.contains("Line 5") || row0_text.contains("Line 6"),
...
fn test_resize_fast_vs_slow_path() {
    ...
    println!(
        "  Before: {}x{}, cursor_y={}",
        before_rows, before_cols, before_cursor_y
    );
    println!(
        "  After: {}x{}, cursor_y={}",
        engine.state.rows, engine.state.cols, engine.state.cursor.y
    );
    println!(
        "  active_transcript_rows: {}",
        get_active_transcript_rows(&engine)
...
fn test_resize_fast_path_rows_only() {
    ...
    println!(
        "  Before: cursor_y={}, first_row={}, transcript={}",
    ...
    println!(
        "  After: cursor_y={}, first_row={}, transcript={}",
    ...
    for row in 0..engine.state.rows {
        ...
        // 每行应该是 "Line XX" 或空
        assert!(
            text.is_empty() || text.contains("Line") || text.trim().is_empty(),
            "Row {} content should be preserved",
            row
        );
    }
}
...
fn test_resize_fast_vs_slow_consistency() {
    ...
    // 光标 Y 应该相同（行数变化相同）
    assert_eq!(
        engine_fast.state.cursor.y, engine_slow.state.cursor.y,
