...
fn test_get_row_bounds_checking() {
    ...
    for row in vec![-(active as i32), -1, 0, rows as i32 - 1] {
        let line_text = engine.state.get_current_screen().get_row(row.into());
    ...
    let selected = engine.state.get_current_screen().get_selected_text(0, (-(active as i32)).into(), 79, (rows as i32 - 1).into());
