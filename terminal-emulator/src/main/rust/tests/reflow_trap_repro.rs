...
fn test_reflow_wide_char_atomicity() {
    ...
    screen
        .get_row_mut(0 as i64)
        .set_char(8, char_zhong as u32, style);
    ...
    let row0_info: String = screen.buffer[idx0]
        .text
        .iter()
        .map(|&c| {
            if c == '\0' {
            ...
            if row.text[c] == char_zhong {
                ...
                if c + 1 < row.text.len() {
                    assert_eq!(
                        row.text[c + 1],
                        '\0',
                        "Wide character and its placeholder split at row {}, col {}",
                        r,
                        c
                    );
                } else {
                    panic!(
                        "Wide character U+4E2D at the very end of row {} with no room for placeholder",
                        r
                    );
                }
            }
...
fn test_reflow_empty_lines_preservation() {
    ...
    assert!(
        found_b >= 2,
        "Empty lines between content should be preserved during reflow"
    );
}
