...
fn test_content_goes_beyond_screen() {
    ...
    let active = engine.state.main_screen.active_transcript_rows as i64;
    ...
    // 验证：最后几行应该在屏幕上
    // 由于有 77 行历史 and 24 行屏幕，Line 100 应该在屏幕底部附近
...
fn test_resize_preserves_history() {
    ...
    println!(
        "After resize to 12 rows: active_transcript={}",
        history_after
    );
