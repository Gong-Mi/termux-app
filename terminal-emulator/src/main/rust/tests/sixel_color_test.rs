...
fn test_color_registers_init() {
    ...
    // 验证颜色寄存器已初始化
    assert_eq!(
        decoder.color_registers.len(),
        256,
        "Should have 256 color registers"
    );
    ...
    for (i, reg) in decoder.color_registers.iter().enumerate() {
        assert!(
            reg.is_none(),
            "Color register {} should be None initially",
            i
        );
    }
...
fn test_color_register_rgb() {
    ...
    // 验证颜色寄存器已设置
    assert!(
        decoder.color_registers[0].is_some(),
        "Color register 0 should be set"
    );
...
fn test_color_register_hls() {
    ...
    assert!(
        decoder.color_registers[1].is_some(),
        "Color register 1 should be set"
    );
    let color = decoder.color_registers[1].as_ref().unwrap();
    println!(
        "  Color 1 (HLS 0,50,100): RGB({}, {}, {})",
...
fn test_sixel_image_with_colors() {
    ...
    assert!(
        decoder.color_registers[0].is_some(),
        "Color 0 should be set"
    );
    assert!(
        decoder.color_registers[1].is_some(),
        "Color 1 should be set"
    );
    assert!(
        decoder.color_registers[2].is_some(),
        "Color 2 should be set"
    );
    ...
    // 验证颜色值
    assert!(
        color0.r > 200 && color0.g < 50 && color0.b < 50,
        "Color 0 should be red"
    );
    assert!(
        color1.g > 200 && color1.r < 50 && color1.b < 50,
        "Color 1 should be green"
    );
    assert!(
        color2.b > 200 && color2.r < 50 && color2.g < 50,
        "Color 2 should be blue"
    );
...
fn test_color_cycling() {
    ...
    for i in 0..16 {
        ...
        assert_eq!(
            decoder.current_color, i as usize,
    ...
    assert_eq!(
        decoder.current_color,
        300 % 256,
        "Color index should wrap at 256"
    );
