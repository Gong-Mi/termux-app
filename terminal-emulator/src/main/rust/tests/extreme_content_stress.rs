...
fn test_massive_50000_rows_stress() {
    ...
    assert!(
        combined_end.contains("Line 45000"),
        "Final line ID must exist in reflowed fragments"
    );
    ...
    assert!(
        last_content.contains("Line 45000"),
        "Final line must be preserved after alternate screen switch"
    );
    ...
    assert!(
        found_mid_anchor,
        "Middle anchor should be preserved even in 50,000 rows buffer"
    );
