...
mod tests {
    ...
    fn test_color_space_mapping() {
        ...
        let scrgb = map_vk_color_space_to_skia(vk::ColorSpaceKHR::EXTENDED_SRGB_LINEAR_EXT).unwrap();
        // 线性伽马
    ...
    fn test_format_priority_logic() {
        ...
        let formats = vec![
            ...
            vk::SurfaceFormatKHR { format: vk::Format::A2B10G10R10_UNORM_PACK32, color_space: vk::ColorSpaceKHR::DISPLAY_P3_NONLINEAR_EXT },
            vk::SurfaceFormatKHR { format: vk::Format::A2B10G10R10_UNORM_PACK32, color_space: vk::ColorSpaceKHR::HDR10_ST2084_EXT },
        ];

        // 模拟我们的优先级选择算法
        let selected = formats.iter()
            .find(|f| f.format == vk::Format::A2B10G10R10_UNORM_PACK32 && f.color_space == vk::ColorSpaceKHR::HDR10_ST2084_EXT)
            .or_else(|| formats.iter().find(|f| f.format == vk::Format::A2B10G10R10_UNORM_PACK32 && f.color_space == vk::ColorSpaceKHR::DISPLAY_P3_NONLINEAR_EXT))
            .or_else(|| formats.iter().find(|f| f.format == vk::Format::A2B10G10R10_UNORM_PACK32))
            .copied()
            .unwrap();
