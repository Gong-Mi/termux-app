...
fn test_screen_expand_content_loss() {
    ...
    println!(
        "   transcript 行数 = {}",
        transcript_after_shrink.lines().count()
    );
    ...
    println!(
        "   transcript 行数 = {}",
        transcript_after_expand.lines().count()
    );
    ...
    println!(
        "   丢失行数：{}",
        expected_lines.saturating_sub(actual_lines)
    );
    ...
    for target in target_lines {
        ...
        let found = transcript_after_expand
            .lines()
            .any(|line| line.contains(&target_str));
        println!(
            "   '{}' - {}",
            target_str,
            if found { "✓ 存在" } else { "✗ 丢失" }
