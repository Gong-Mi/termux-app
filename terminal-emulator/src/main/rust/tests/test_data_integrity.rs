...
fn test_data_integrity_java_to_rust() {
    ...
    // 2. 测试 UTF-8 多字节字符
    println!("\n2. 测试 UTF-8 多字节字符:");
    engine = TerminalEngine::new(80 as i64, 24 as i64, 1000, 10, 20);
    ...
    if let Some(first) = transcript4.lines().next() {
        assert!(
            first.contains("Line 0000") || first.contains("Line 0001"),
            "第一行应该是 Line 0000 或 0001，实际：{}",
            first
        );
    }
    if let Some(last) = transcript4.lines().last() {
        assert!(
            last.contains("Line 0999"),
            "最后一行应该是 Line 0999，实际：{}",
            last
        );
    }
