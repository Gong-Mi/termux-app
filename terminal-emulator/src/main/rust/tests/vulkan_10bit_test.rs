...
fn test_10bit_creation() {
    ...
    let (img1, _) = vk.create_image(width, height, vk::Format::A2B10G10R10_UNORM_PACK32).unwrap();
    let res1 = create_skia_surface(&vk, img1, width, height, vk::Format::A2B10G10R10_UNORM_PACK32, ColorType::RGBA1010102, None);
    println!("创建 10-bit Surface (无 ColorSpace): {}", if res1.is_some() { "✅ 成功" } else { "❌ 失败" });

    // 测试 2: 创建 10-bit Surface (sRGB ColorSpace)
    let (img2, _) = vk.create_image(width, height, vk::Format::A2B10G10R10_UNORM_PACK32).unwrap();
    let srgb = skia_safe::ColorSpace::new_srgb();
    let res2 = create_skia_surface(&vk, img2, width, height, vk::Format::A2B10G10R10_UNORM_PACK32, ColorType::RGBA1010102, Some(srgb));
    println!("创建 10-bit Surface (sRGB ColorSpace): {}", if res2.is_some() { "✅ 成功" } else { "❌ 失败" });

    // 测试 3: 创建 10-bit Surface (sRGB Linear ColorSpace)
    let (img3, _) = vk.create_image(width, height, vk::Format::A2B10G10R10_UNORM_PACK32).unwrap();
    let srgb_linear = skia_safe::ColorSpace::new_srgb_linear();
    let res3 = create_skia_surface(&vk, img3, width, height, vk::Format::A2B10G10R10_UNORM_PACK32, ColorType::RGBA1010102, Some(srgb_linear));
    println!("创建 10-bit Surface (sRGB Linear ColorSpace): {}", if res3.is_some() { "✅ 成功" } else { "❌ 失败" });
}
