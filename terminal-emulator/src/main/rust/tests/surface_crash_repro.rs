...
impl MockVulkanContext {
    ...
    fn present(&self) {
        ...
        if !self.is_valid.load(Ordering::SeqCst) {
            // 在真实环境中，这里会是 SIGSEGV
            panic!(
                "CRASH: SIGSEGV at fault addr 0x5b0! Accessing invalidated ANativeWindow handle for window {}",
                self.window_id
            );
        }
        ...
        if !self.is_valid.load(Ordering::SeqCst) {
            panic!(
                "CRASH: SIGSEGV during queue_present! Surface was destroyed while GPU was active."
            );
        }
    }
}
...
impl Drop for MockVulkanContext {
    fn drop(&mut self) {
        println!(
            "MockVulkanContext: Dropping resources for window {}",
            self.window_id
        );
...
fn test_surface_destruction_race_condition() {
    ...
    let context = Arc::new(Mutex::new(Some(MockVulkanContext::new(
        1001,
        window_valid.clone(),
    ))));
    ...
    match result {
        ...
        Err(e) => {
            ...
            assert!(
                msg.contains("CRASH"),
                "Panic should represent a simulated crash"
            );
        }
    }
