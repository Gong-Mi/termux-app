...
fn test_extreme_shrinking_reflow() {
    ...
    let line =
        "This is a long line that will be wrapped many times when the screen is shrunk extreme.";
...
fn test_realistic_reflow_with_history() {
    ...
    println!(
        "Row 23 content (current screen bottom): '{}'",
        get_row_text(&engine, 23)
    );
    ...
    assert!(
        all_text.contains("History Line 049"),
        "Combined text should contain the last written line '049'"
    );
}
