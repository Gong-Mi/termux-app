...
fn test_reflow_displacement_accuracy() {
    ...
    assert_eq!(new_cy, 2, "Displacement Bug: Cursor Y should have moved down due to wrapping above");
    assert_eq!(new_cx, 0, "Cursor X should be 0");
    
    // 验证文字内容是否还在
    let row2_idx = screen.internal_row(2);
    assert_eq!(screen.buffer[row2_idx].text[0], ' ', "Row 2 should start with space (cursor line)");
}
...
fn test_reflow_line_wrap_preservation() {
    ...
    for i in 0..10 { screen.buffer[0].text[i] = 'X'; }
    // 模拟它是一个逻辑行的开始
    ...
    assert!(screen.buffer[idx0].line_wrap, "Line 0 should still be wrapped");
    assert!(screen.buffer[idx1].line_wrap, "Line 1 (continuation) should be wrapped");
}
...
fn test_reflow_latency_stress() {
    ...
    assert!(duration.as_millis() < 30, "Latency Bug: Reflow took too long ({:?})", duration);
}
...
fn test_screen_misalignment_after_scroll() {
    ...
    assert!(found, "Misalignment Bug: Content 'HI' lost or moved to invalid memory after reflow with non-zero first_row");
}
