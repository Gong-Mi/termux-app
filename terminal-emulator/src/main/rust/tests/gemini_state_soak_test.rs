...
fn test_gemini_session_lifecycle_soak() {
    ...
    for i in 1..=100 {
        ...
        engine
            .process_bytes(b"\x1b[8m\x1b[?u\x1b]11;?\x1b\\\x1b[>q\x1b[>4;2m\x1b[c\x1b[2K\r\x1b[0m");

        // 校验：探测后样式必须回到 NORMAL，不应残留隐藏模式(8m)或被探测指令污染
        let after_query = StateSnapshot::capture(&engine);
        assert_eq!(
            after_query.style, STYLE_NORMAL,
            "第 {} 轮：探测序列后样式未重置",
            i
        );
        ...
        assert_ne!(
            mid_session.style, STYLE_NORMAL,
            "第 {} 轮：UI 内容未正确设置样式",
            i
        );
        ...
        assert_eq!(
            current.use_alt_buffer, baseline.use_alt_buffer,
            "第 {} 轮：缓冲区泄漏",
            i
        );
        assert_eq!(current.style, baseline.style, "第 {} 轮：样式状态泄漏", i);
        assert_eq!(
            current.decset_flags, baseline.decset_flags,
            "第 {} 轮：DECSET 模式泄漏",
            i
        );
...
fn test_style_pollution_edge_cases() {
    ...
    assert_eq!(
        engine.state.current_style, STYLE_NORMAL,
        "混合前缀 ?> 应该被视为私有指令并忽略"
    );
