...
fn get_cursor_position(engine: &TerminalEngine) -> (i32, i32) {
    (engine.state.cursor.x as i32, engine.state.cursor.y as i32)
}
...
fn test_up_arrow_key() {
    ...
    assert!(
        row0.contains("$") || row0.contains("ls"),
...
fn test_continuous_up_arrow_history_navigation() {
    ...
    assert_eq!(
        cursor_before,
        (2, 3),
        "Initial cursor should be at column 2, row 3"
    );
    ...
    for i in 1..=3 {
        ...
        println!(
            "  After UP press {}: cursor={:?}",
            i,
            get_cursor_position(&engine)
        );
    }
    ...
    assert!(
        row3.starts_with("$"),
        "Last line should still show prompt, got: '{}'",
        row3
    );
...
fn test_cursor_application_mode_arrows() {
    ...
    assert!(
        ...
        "Cursor application mode should be enabled"
    );
...
fn test_alt_char_combination() {
    ...
    let result = engine
        .state
        .send_key_event(0, Some("b".to_string()), meta_state);

    assert_eq!(
        result,
        Some("\x1bb".to_string()),
        "Alt+b should generate ESC+b sequence"
    );
