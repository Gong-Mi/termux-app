...
fn isolated(name: &str) -> bool {
    if std::env::var_os("TERMUX_PROCESS_TEST_CHILD").is_some() {
        return false;
    }
    let out = Command::new(std::env::current_exe().unwrap())
        .args(["--exact", name, "--test-threads=1"])
        .env("TERMUX_PROCESS_TEST_CHILD", "1")
        .output()
        .unwrap();
    assert!(
        out.status.success(),
...
fn shell(command: &str) -> Command {
    let path = if std::path::Path::new("/system/bin/sh").exists() {
        "/system/bin/sh"
    } else {
        "/bin/sh"
    };
...
fn finished(coordinator: &SessionCoordinator, session: usize) {
    ...
    while coordinator.get_session_state(session) != Some(SessionState::Finished) {
        assert!(
            ...
            "registered process exit was lost"
        );
...
fn unrelated_child_remains_waitable_by_its_real_owner() {
    if isolated("unrelated_child_remains_waitable_by_its_real_owner") {
        return;
    }
    ...
    assert_eq!(
        child
            .wait()
            .expect("coordinator stole an unrelated child")
            .code(),
        Some(7)
    );
}
...
fn exit_before_bind_is_retained_not_overwritten_with_running() {
    if isolated("exit_before_bind_is_retained_not_overwritten_with_running") {
        return;
    }
...
fn late_bind_cannot_resurrect_unregistered_session() {
    if isolated("late_bind_cannot_resurrect_unregistered_session") {
        return;
    }
    let coordinator = SessionCoordinator::get();
    let session = coordinator.register_session();
...
fn pkg_release_cannot_turn_exited_process_back_into_running() {
    if isolated("pkg_release_cannot_turn_exited_process_back_into_running") {
        return;
    }
    let coordinator = SessionCoordinator::get();
    let session = coordinator.register_session();
    let mut child = shell("read -r value; exit 0")
        .stdin(Stdio::piped())
        .spawn()
        .unwrap();
    let _ = coordinator.bind_pid(session, child.id() as i32);
...
fn terminate_before_bind_is_applied_to_the_owned_child() {
    if isolated("terminate_before_bind_is_applied_to_the_owned_child") {
        return;
    }
    let coordinator = SessionCoordinator::get();
    let session = coordinator.register_session();
    ...
    let child = shell("read -r value; exit 0")
        .stdin(Stdio::piped())
        .spawn()
        .unwrap();
    ...
    assert_eq!(
        owner.wait(),
        termux_rust::process_owner::ExitOutcome::Exited(-libc::SIGKILL)
    );
    finished(coordinator, session);
    assert_eq!(
        coordinator.process_status(session),
        Some([2, -1, -libc::SIGKILL])
    );
    assert!(!coordinator.terminate_session(session).unwrap());
    coordinator.unregister_session(session);
    assert!(!coordinator.terminate_session(session).unwrap());
...
fn unregister_does_not_allow_a_second_owner_to_steal_the_live_child() {
    if isolated("unregister_does_not_allow_a_second_owner_to_steal_the_live_child") {
        return;
    }
    let coordinator = SessionCoordinator::get();
    let session = coordinator.register_session();
    ...
    assert_eq!(coordinator.get_session_state(session), None);
    assert_eq!(
        coordinator.get_session_state(other),
...
fn legacy_managed_waits_return_cached_status_without_double_reaping() {
    if isolated("legacy_managed_waits_return_cached_status_without_double_reaping") {
        return;
    }
    let coordinator = SessionCoordinator::get();
    let session = coordinator.register_session();
    let mut child = shell("read -r value; exit 19")
        .stdin(Stdio::piped())
        .spawn()
        .unwrap();
    let pid = child.id() as i32;
    ...
    assert_eq!(
        owner.wait(),
        termux_rust::process_owner::ExitOutcome::Exited(19)
    );
    finished(coordinator, session);
    assert!(!coordinator.try_acquire_pkg_lock(session));
    coordinator.unregister_session(session);
...
fn known_exit_rejects_new_input_but_reader_keeps_parsing_tail() {
    if isolated("known_exit_rejects_new_input_but_reader_keeps_parsing_tail") {
        return;
    }
    ...
    assert_eq!(
        process.wait(),
        termux_rust::process_owner::ExitOutcome::Exited(0)
    );
    ...
    loop {
        ...
        if String::from_utf16_lossy(&row).starts_with("tail-after-exit") {
            break;
        }
...
fn late_polling_delivery_reclaims_owner_instead_of_resurrecting_session() {
    if isolated("late_polling_delivery_reclaims_owner_instead_of_resurrecting_session") {
        return;
    }
    use termux_rust::engine::{ENGINE_HANDLES, TerminalContext, TerminalEngine};
    let coordinator = SessionCoordinator::get();
    let session = coordinator.register_session();
    coordinator.unregister_session(session);
    let context = std::sync::Arc::new(TerminalContext::new(TerminalEngine::new(
        0, 80, 24, 2000, 8, 16,
    )));
    let handle = ENGINE_HANDLES.insert(context).unwrap();
    coordinator.set_engine_data(
        session,
        termux_rust::coordinator::SessionEngineData {
            ptr: handle,
            pty_fd: -1,
            pid: -1,
        },
