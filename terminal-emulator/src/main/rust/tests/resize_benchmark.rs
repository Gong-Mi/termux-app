...
fn benchmark_resize_comparison() {
    ...
    if speedup > 10.0 {
        println!(
            "\n  ✅ Fast path is {:.1}x faster - optimization successful!",
            speedup
        );
    } else if speedup > 5.0 {
        println!(
            "\n  ✅ Fast path is {:.1}x faster - good improvement!",
            speedup
        );
    } else {
        println!(
            "\n  ⚠️  Fast path is only {:.1}x faster - expected more speedup",
            speedup
        );
    }
}
