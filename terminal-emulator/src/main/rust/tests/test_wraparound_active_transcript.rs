...
fn test_active_transcript_rows_wraparound() {
    ...
    // 1. 创建小缓冲区测试绕回
    let mut engine = TerminalEngine::new(80 as i64, 10 as i64, 100, 10, 20); // 只有 100 行缓冲区
    ...
    println!("\n3. 读取历史行 {}: ", min_row);
    let first_history = engine.state.get_current_screen().get_row(min_row as i64);
    let text: String = first_history
        .text
        .iter()
        .take(50)
        .filter(|&&c| c != '\0')
    ...
    if let Some(last_line) = transcript.lines().last() {
        println!(
            "   最后一行：'{}'",
            last_line.chars().take(50).collect::<String>()
        );
    }
