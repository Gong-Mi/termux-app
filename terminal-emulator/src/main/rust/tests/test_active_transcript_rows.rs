...
fn test_active_transcript_rows_increment_on_scroll() {
    ...
    let mut engine = TerminalEngine::new(80 as i64, 10 as i64, 20, 10, 20);

    // 初始状态：没有历史
    assert_eq!(
        engine.state.main_screen.active_transcript_rows, 0,
        "初始 active_transcript_rows 应为 0"
    );
    println!(
        "初始状态：active_transcript_rows = {}",
    ...
    assert_eq!(
        engine.state.main_screen.active_transcript_rows, 0,
        "未填满屏幕时 active_transcript_rows 应为 0"
    );
    println!(
        "写入 9 行后：active_transcript_rows = {}",
        engine.state.main_screen.active_transcript_rows
    );
    ...
    assert_eq!(
        engine.state.main_screen.active_transcript_rows, 0,
        "填满屏幕但未换行时 active_transcript_rows 应为 0"
    );
    println!(
        "写入第 10 行（无换行）：active_transcript_rows = {}",
        engine.state.main_screen.active_transcript_rows
    );
    ...
    assert_eq!(
        engine.state.main_screen.active_transcript_rows, 1,
        "换行后 active_transcript_rows 应为 1"
    );
    println!(
        "换行后：active_transcript_rows = {}",
        engine.state.main_screen.active_transcript_rows
    );
    ...
    assert_eq!(
        engine.state.main_screen.active_transcript_rows, 6,
        "继续写入 5 行后 active_transcript_rows 应为 6"
    );
    println!(
        "写入 15 行后：active_transcript_rows = {}",
        engine.state.main_screen.active_transcript_rows
    );
...
fn test_active_transcript_rows_max_limit() {
    ...
    let mut engine = TerminalEngine::new(80 as i64, 5 as i64, 10, 10, 20);
    ...
    assert_eq!(
        ...
        "active_transcript_rows 不应超过最大值 {}",
        max_transcript
    );
    println!(
        "写入 20 行后：active_transcript_rows = {} (最大值：{})",
        engine.state.main_screen.active_transcript_rows, max_transcript
    );
    ...
    println!(
        "写入 30 行后：active_transcript_rows = {} (保持不变)",
        engine.state.main_screen.active_transcript_rows
    );
...
fn test_active_transcript_rows_on_resize() {
    ...
    println!(
        "缩小到 8 行后：active_transcript_rows = {}",
        transcript_after_shrink
    );
    assert!(
        transcript_after_shrink >= transcript_before,
        "缩小屏幕后历史行数应增加或保持不变"
    );
    ...
    println!(
        "扩大到 12 行后：active_transcript_rows = {}",
        transcript_after_expand
    );
    assert!(
        transcript_after_expand <= transcript_after_shrink,
        "扩大屏幕后历史行数应减少或保持不变"
    );
...
fn test_active_transcript_rows_alt_screen() {
    ...
    println!(
        "备用屏幕：active_transcript_rows = {}",
        alt_screen_transcript
    );
    assert_eq!(
        alt_screen_transcript, 0,
        "备用屏幕的 active_transcript_rows 应为 0"
    );
    ...
    println!(
        "切回主屏幕：active_transcript_rows = {}",
        main_transcript_after
    );
