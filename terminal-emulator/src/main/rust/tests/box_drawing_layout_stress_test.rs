...
mod tests {
    ...
    fn test_box_drawing_with_float_scaling_and_offset() {
        ...
        println!("Float alignment: Cell1_Exit={}, Cell2_Entry={}", cell1_right_exit, cell2_left_entry);
        
        // 使用极小值 epsilon 进行浮点比较
        assert!((cell1_right_exit - cell2_left_entry).abs() < 1e-6, "浮点缩放导致衔接裂缝！");
        ...
        let get_y_top = |row_idx: i32, scroll_offset: i32, h: f32| {
            (row_idx - scroll_offset) as f32 * h
        };

        let y_at_scroll_0 = get_y_top(10, 0, font_h);
        let y_at_scroll_5 = get_y_top(10, 5, font_h);
        
        println!("Scrolling y_top: scroll_0={}, scroll_5={}", y_at_scroll_0, y_at_scroll_5);
        assert_eq!(y_at_scroll_0 - (5.0 * font_h), y_at_scroll_5, "滚动偏移量未线性对应");
    }
