...
fn proof_sgr_pollution() {
    ...
    assert!(
        ...
        "Initial state should not have underline"
    );
    ...
    assert!(
        !has_underline,
        "Detection sequence polluted SGR state (Underline effect triggered)"
    );
}
...
fn proof_content_leak() {
    ...
    assert!(
        !line1_exists,
        "Old content leaked after Gemini startup sequence"
    );
}
