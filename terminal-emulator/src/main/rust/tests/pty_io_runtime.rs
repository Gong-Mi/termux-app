...
fn socket_pair() -> (OwnedFd, OwnedFd) {
    let mut fds = [-1; 2];
    assert_eq!(unsafe { libc::socketpair(libc::AF_UNIX, libc::SOCK_STREAM | libc::SOCK_CLOEXEC, 0, fds.as_mut_ptr()) }, 0);
    unsafe { (OwnedFd::from_raw_fd(fds[0]), OwnedFd::from_raw_fd(fds[1])) }
}
fn write_small(fd: &OwnedFd, bytes: &[u8]) {
    assert_eq!(unsafe { libc::write(fd.as_raw_fd(), bytes.as_ptr().cast(), bytes.len()) }, bytes.len() as isize);
}
fn read_exact(fd: &OwnedFd, len: usize) -> Vec<u8> {
    let deadline = Instant::now() + Duration::from_secs(5);
    let mut output = Vec::new();
    while output.len() < len {
        assert!(Instant::now() < deadline, "read timeout: {} of {len}", output.len());
        ...
        assert!(n > 0, "unexpected EOF/error: {:?}", io::Error::last_os_error());
        output.extend_from_slice(&buffer[..n as usize]);
    }
    output
}
fn gate() -> (std::sync::mpsc::Sender<()>, std::sync::mpsc::Receiver<()>) { std::sync::mpsc::channel() }
...
fn queued_and_in_flight_capacity_is_all_or_none() {
    ...
    runtime.cancel(); stopped(&runtime); runtime.join().unwrap();
}
...
fn eof_delivers_final_read_to_callback_before_stopping() {
    ...
    let mut runtime = IoRuntime::start(master, 4096, move |bytes| {
        sink.lock().unwrap().extend_from_slice(bytes); vec![]
    }).unwrap();
    write_small(&peer, b"final-\xf0\x9f\x8c\x99");
    assert_eq!(unsafe { libc::shutdown(peer.as_raw_fd(), libc::SHUT_WR) }, 0);
    stopped(&runtime);
...
fn parser_response_overflow_is_a_terminal_failure_not_silent_loss() {
    ...
    let mut runtime = IoRuntime::start_with_callbacks(master, 4,
        |_| vec![b"12345".to_vec()], || {}, move |outcome| {
            notified.send(matches!(outcome, StopOutcome::ResponseOverflow)).unwrap();
        }).unwrap();
    write_small(&slave, b"query");
    assert!(observed.recv_timeout(Duration::from_secs(3)).unwrap(), "failure reason must be visible before join");
    stopped(&runtime);
    assert!(matches!(runtime.join().unwrap(), StopOutcome::ResponseOverflow));
    assert_eq!(runtime.submit(b"x"), Err(SubmitError::Closed));
}
...
fn callback_unwind_closes_fd_and_old_cancel_never_closes_reused_number() {
    ...
    if std::env::var_os("TERMUX_PTY_FD_REUSE_CHILD").is_none() {
        let output = std::process::Command::new(std::env::current_exe().unwrap())
            .args(["--exact", "callback_unwind_closes_fd_and_old_cancel_never_closes_reused_number", "--test-threads=1"])
            .env("TERMUX_PTY_FD_REUSE_CHILD", "1").output().unwrap();
        assert!(output.status.success(), "{}{}", String::from_utf8_lossy(&output.stdout), String::from_utf8_lossy(&output.stderr));
        return;
    }
    let (master, slave) = pty();
    let fd = master.as_raw_fd();
    let mut runtime = IoRuntime::start(master, 4096, |_| panic!("injected callback panic")).unwrap();
    write_small(&slave, b"x"); stopped(&runtime);
    ...
    let replacement_fd = if replacement.as_raw_fd() == fd { None } else {
        assert_eq!(unsafe { libc::dup2(replacement.as_raw_fd(), fd) }, fd);
        Some(unsafe { OwnedFd::from_raw_fd(fd) })
    };
    runtime.cancel(); runtime.cancel(); drop(runtime);
    assert!(unsafe { libc::fcntl(fd, libc::F_GETFD) } >= 0);
    drop(replacement_fd); drop(replacement);
}
...
fn resize_is_executed_by_worker_on_real_pty() {
    ...
    loop {
        ...
        assert_eq!(unsafe { libc::ioctl(slave.as_raw_fd(), libc::TIOCGWINSZ, &mut size) }, 0);
        if (size.ws_row, size.ws_col, size.ws_xpixel, size.ws_ypixel) == (25,81,648,400) { break; }
...
fn invalid_resize_target_reports_io_error_and_closes_admission() {
    ...
    let mut runtime = IoRuntime::start(master, 4096, |_| vec![]).unwrap();
    runtime.resize(1,1,1,1).unwrap(); stopped(&runtime);
...
fn fresh_master_waits_for_first_slave_open_without_eof_debounce() {
    ...
    assert_eq!(unsafe { libc::ptsname_r(raw, name.as_mut_ptr(), name.len()) }, 0);
    let (sent, received) = std::sync::mpsc::channel();
    let mut runtime = IoRuntime::start(master, 4096, move |bytes| { sent.send(bytes.to_vec()).unwrap(); vec![] }).unwrap();
    // Mirror create_subprocess's parent-return before child opens the slave.
    std::thread::sleep(Duration::from_millis(20));
    assert!(!runtime.is_stopped(), "master falsely finished before first slave open");
    let name = unsafe { CStr::from_ptr(name.as_ptr()) };
    let slave_fd = unsafe { libc::open(name.as_ptr(), libc::O_RDWR | libc::O_CLOEXEC) };
    assert!(slave_fd >= 0);
    let slave = unsafe { OwnedFd::from_raw_fd(slave_fd) };
    write_small(&slave, b"first-open");
    assert_eq!(received.recv_timeout(Duration::from_secs(3)).unwrap(), b"first-open");
    runtime.cancel(); stopped(&runtime); runtime.join().unwrap();
}
...
fn cancel_revokes_admission_but_does_not_pretend_to_interrupt_callback() {
    ...
    assert_eq!(runtime.submit(b"x"), Err(SubmitError::Closed));
    release.send(()).unwrap(); stopped(&runtime);
    assert!(matches!(runtime.join().unwrap(), StopOutcome::Cancelled));
}
...
fn replies_enter_fifo_before_notification_reentrant_input() {
    ...
    let (entered_tx, entered) = gate();
    let (release, released) = gate();
    let mut runtime = IoRuntime::start_with_callbacks(master, 4096,
        |_| vec![b"reply".to_vec()], move || {
            entered_tx.send(()).unwrap();
            released.recv_timeout(Duration::from_secs(3)).unwrap();
        }, |_| {}).unwrap();
    write_small(&slave, b"query");
    ...
    release.send(()).unwrap();
    assert_eq!(read_exact(&slave, b"replycallback-input".len()), b"replycallback-input");
    runtime.cancel(); stopped(&runtime); runtime.join().unwrap();
}
