...
fn test_inverse_video_basic() {
    ...
    let mut engine_reset = TerminalEngine::new(80 as i64, 24 as i64, 100, 10, 20);
...
fn test_inverse_video_in_text() {
    ...
    engine.state.copy_row_text(0 as i64, &mut text_buffer);
...
fn test_all_sgr_effects() {
    ...
    assert!(
        (engine.state.effect & EFFECT_BOLD) != 0,
        "SGR 1 应该开启粗体"
    );
    ...
    assert!(
        (engine.state.effect & EFFECT_ITALIC) != 0,
        "SGR 3 应该开启斜体"
    );
    ...
    assert!(
        (engine.state.effect & EFFECT_UNDERLINE) != 0,
        "SGR 4 应该开启下划线"
    );
    ...
    assert!(
        (engine.state.effect & EFFECT_BLINK) != 0,
        "SGR 5 应该开启闪烁"
    );
    ...
    assert!(
        (engine.state.effect & EFFECT_INVISIBLE) != 0,
        "SGR 8 应该开启不可见"
    );
    ...
    assert!(
        (engine.state.effect & EFFECT_STRIKETHROUGH) != 0,
        "SGR 9 应该开启删除线"
    );
    ...
    engine.process_bytes(b"\x1b[7m");
    assert!(
        (engine.state.effect & EFFECT_REVERSE) != 0,
        "SGR 7 应该开启反色"
    );
    engine.process_bytes(b"\x1b[27m");
    assert!(
...
fn test_decset5_reverse_video() {
    ...
    // 初始状态
    let initial_reverse_video = engine
        .state
        .modes
        .is_enabled(DECSET_BIT_REVERSE_VIDEO as i32);
    assert!(!initial_reverse_video, "初始状态不应该有全局反色");
    println!("  初始全局反色：关闭");

    // 开启 DECSET 5
    engine.process_bytes(b"\x1b[?5h");
    let reverse_video_on = engine
        .state
        .modes
        .is_enabled(DECSET_BIT_REVERSE_VIDEO as i32);
