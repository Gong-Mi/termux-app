...
impl ProcessOwner {
    ...
    pub fn has_pidfd_wait(&self) -> bool {
        self.lock().pidfd_wait
    }
...
mod tests {
    ...
    fn pidfd_wait_does_not_follow_a_reassigned_numeric_label() {
        ...
        if !owner.has_pidfd_wait() {
            ...
            owner.wait();
            assert_eq!(
                original.wait().unwrap_err().raw_os_error(),
                Some(libc::ECHILD)
            );
            return;
        }
        let mut unrelated = Command::new("sh")
            .args(["-c", "sleep 0.1; exit 77"])
            .spawn()
            .unwrap();
