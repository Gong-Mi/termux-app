    ...
    std::thread::spawn(move || {
        let result = std::panic::catch_unwind(move || {
            ...
            if let Some(cb) = callback_ref.clone() {
                let installed = coordinator.install_completion_sink(session_id, Arc::new(move |candidate| {
                    use crate::process_owner::ExitOutcome;
                    use crate::engine::io_runtime::IoOutcome;
                    let Some(vm) = crate::JAVA_VM.get() else { return false; };
                    let Ok(mut env) = vm.get_env().or_else(|_| vm.attach_current_thread_as_daemon()) else { return false; };
                    ...
                    let result = env.call_method(cb.as_obj(), "onSessionCompletion", "(IIIII)Z", &[
                        jni::objects::JValue::Int(candidate.session_id as i32),
                        jni::objects::JValue::Int(pk), jni::objects::JValue::Int(pc),
                        jni::objects::JValue::Int(ik), jni::objects::JValue::Int(ic),
