...
use std::{collections::VecDeque, io, os::fd::{AsRawFd, FromRawFd, OwnedFd},
    sync::{Arc, Mutex, atomic::{AtomicBool, Ordering}}, thread::{self, JoinHandle}};

#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub enum SubmitError { Closed, Full }

#[derive(Debug)]
pub enum StopOutcome { Eof, Cancelled, IoError(io::Error), ResponseOverflow }
...
impl IoRuntime {
    pub fn start(fd: OwnedFd, capacity: usize,
        on_bytes: impl FnMut(&[u8]) -> Vec<Vec<u8>> + Send + 'static) -> io::Result<Self> {
    ...
    pub fn start_with_callbacks(fd: OwnedFd, capacity: usize,
        on_bytes: impl FnMut(&[u8]) -> Vec<Vec<u8>> + Send + 'static,
        after_read: impl FnMut() + Send + 'static,
        on_stop: impl FnOnce(&StopOutcome) + Send + 'static) -> io::Result<Self> {
        ...
        if wake < 0 { return Err(io::Error::last_os_error()); }
        let shared = Arc::new(Shared {
            pending: Mutex::new(Pending { queue: VecDeque::new(), bytes: 0, resize: None }),
            capacity, closed: AtomicBool::new(false), stopped: AtomicBool::new(false),
            wake: unsafe { OwnedFd::from_raw_fd(wake) },
        });
        let state = shared.clone();
        let worker = thread::Builder::new().name("pty-io".into()).spawn(move || {
            let owner = WorkerOwner { fd: Some(fd), shared: state };
    ...
    pub fn submit(&self, data: &[u8]) -> Result<(), SubmitError> {
        ...
        if self.shared.closed.load(Ordering::Acquire) { return Err(SubmitError::Closed); }
        if data.len() > self.shared.capacity - pending.bytes { return Err(SubmitError::Full); }
        if !data.is_empty() {
    ...
    pub fn resize(&self, rows: u16, cols: u16, xpixel: u16, ypixel: u16) -> Result<(), SubmitError> {
        let mut pending = self.shared.pending.lock().unwrap();
        if self.shared.closed.load(Ordering::Acquire) { return Err(SubmitError::Closed); }
        pending.resize = Some(libc::winsize { ws_row: rows, ws_col: cols, ws_xpixel: xpixel, ws_ypixel: ypixel });
        drop(pending);
        self.shared.wake();
        Ok(())
    }
    ...
    pub fn is_stopped(&self) -> bool { self.shared.stopped.load(Ordering::Acquire) }
    /// Background/test use only. A second join is a programming error and panics.
    pub fn join(&mut self) -> thread::Result<StopOutcome> {
        self.worker.take().expect("PTY worker already joined").join()
    }
}
impl Drop for IoRuntime { fn drop(&mut self) { self.cancel(); } }
...
struct WorkerOwner { fd: Option<OwnedFd>, shared: Arc<Shared> }
impl Drop for WorkerOwner {
    fn drop(&mut self) {
        ...
        pending.queue.clear(); pending.bytes = 0; pending.resize = None;
        drop(pending);
...
fn run(owner: &WorkerOwner, mut on_bytes: impl FnMut(&[u8]) -> Vec<Vec<u8>>, mut after_read: impl FnMut()) -> StopOutcome {
    ...
    loop {
        if shared.closed.load(Ordering::Acquire) { return StopOutcome::Cancelled; }
        ...
        if head.is_none() { head = shared.pending.lock().unwrap().queue.pop_front().map(|v| (v, 0)); }
        let mut polls = [
            libc::pollfd { fd, events: libc::POLLIN | if head.is_some() { libc::POLLOUT } else { 0 }, revents: 0 },
            libc::pollfd { fd: shared.wake.as_raw_fd(), events: libc::POLLIN, revents: 0 },
        ];
        ...
        if polls[1].revents & libc::POLLIN != 0 {
            ...
            unsafe { libc::read(shared.wake.as_raw_fd(), (&mut value as *mut u64).cast(), 8); }
        }
        ...
        if ready & (libc::POLLIN | libc::POLLHUP | libc::POLLERR) != 0 {
            ...
            if n == 0 { return StopOutcome::Eof; }
            if n < 0 {
                let error = io::Error::last_os_error();
                if tty && error.raw_os_error() == Some(libc::EIO) { return StopOutcome::Eof; }
                if !retryable(&error) { return StopOutcome::IoError(error); }
            } else {
                if shared.closed.load(Ordering::Acquire) { return StopOutcome::Cancelled; }
                let responses = on_bytes(&input[..n as usize]);
                let mut pending = shared.pending.lock().unwrap();
                if shared.closed.load(Ordering::Acquire) { return StopOutcome::Cancelled; }
                let total = responses.iter().try_fold(0usize, |sum, v| sum.checked_add(v.len()));
                let Some(total) = total.filter(|n| *n <= shared.capacity - pending.bytes) else {
                    return StopOutcome::ResponseOverflow;
                };
                pending.bytes += total;
                pending.queue.extend(responses.into_iter().filter(|v| !v.is_empty()));
                drop(pending);
                after_read();
            }
        }
        ...
        if ready & libc::POLLOUT != 0 && ready & libc::POLLHUP == 0 {
            if let Some((bytes, offset)) = head.as_mut() {
                ...
                if n < 0 {
                    ...
                    return StopOutcome::IoError(io::Error::new(io::ErrorKind::WriteZero, "PTY write returned zero"));
                } else {
                    *offset += n as usize;
                    shared.pending.lock().unwrap().bytes -= n as usize;
                    if *offset == bytes.len() { head = None; }
                }
            }
        }
    }
