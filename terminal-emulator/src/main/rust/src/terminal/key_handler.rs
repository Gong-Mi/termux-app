...
fn init_termcap_map() -> HashMap<&'static str, u32> {
    ...
    map.insert("kP", KEYCODE_PAGE_UP as u32);
    map.insert("kN", KEYCODE_PAGE_DOWN as u32);
...
pub fn get_code(
    ...
    match key_code {
        ...
        KEYCODE_DEL => {
            let prefix = if (key_mode & KEYMOD_ALT) == 0 {
                ""
            } else {
                "\x1b"
            };
...
fn transform_for_modifiers(start: &str, keymod: u32, last_char: char) -> String {
    ...
    if (keymod & KEYMOD_SHIFT) != 0 {
        modifier += 1;
    }
    if (keymod & KEYMOD_ALT) != 0 {
        modifier += 2;
    }
    if (keymod & KEYMOD_CTRL) != 0 {
        modifier += 4;
    }
...
mod tests {
    ...
    fn test_arrow_keys() {
        // 无修饰符的方向键
        assert_eq!(
            get_code(KEYCODE_DPAD_UP, 0, false, false),
            Some("\x1b[A".to_string())
        );
        assert_eq!(
            get_code(KEYCODE_DPAD_DOWN, 0, false, false),
            Some("\x1b[B".to_string())
        );
        assert_eq!(
            get_code(KEYCODE_DPAD_RIGHT, 0, false, false),
            Some("\x1b[C".to_string())
        );
        assert_eq!(
            get_code(KEYCODE_DPAD_LEFT, 0, false, false),
    ...
    fn test_cursor_application_mode() {
        // 应用光标模式
        assert_eq!(
            get_code(KEYCODE_DPAD_UP, 0, true, false),
            Some("\x1bOA".to_string())
        );
        assert_eq!(
            get_code(KEYCODE_DPAD_DOWN, 0, true, false),
    ...
    fn test_function_keys() {
        // F1-F4 基础
        assert_eq!(
            get_code(KEYCODE_F1, 0, false, false),
            Some("\x1bOP".to_string())
        );
        assert_eq!(
            get_code(KEYCODE_F2, 0, false, false),
            Some("\x1bOQ".to_string())
        );
        assert_eq!(
            get_code(KEYCODE_F3, 0, false, false),
            Some("\x1bOR".to_string())
        );
        assert_eq!(
            get_code(KEYCODE_F4, 0, false, false),
            Some("\x1bOS".to_string())
        );

        // F5-F12
        assert_eq!(
            get_code(KEYCODE_F5, 0, false, false),
            Some("\x1b[15~".to_string())
        );
        assert_eq!(
            get_code(KEYCODE_F12, 0, false, false),
    ...
    fn test_modifiers() {
        // Shift + ↑
        assert_eq!(
            get_code(KEYCODE_DPAD_UP, KEYMOD_SHIFT, false, false),
            Some("\x1b[1;2A".to_string())
        );
        // Ctrl + ↑
        assert_eq!(
            get_code(KEYCODE_DPAD_UP, KEYMOD_CTRL, false, false),
            Some("\x1b[1;5A".to_string())
        );
        // Alt + ↑
        assert_eq!(
            get_code(KEYCODE_DPAD_UP, KEYMOD_ALT, false, false),
            Some("\x1b[1;3A".to_string())
        );
        // Ctrl + Shift + ↑ (modifier 6)
        assert_eq!(
            get_code(KEYCODE_DPAD_UP, KEYMOD_CTRL | KEYMOD_SHIFT, false, false),
            Some("\x1b[1;6A".to_string())
        );
        // Alt + Shift + ↑ (modifier 4)
        assert_eq!(
            get_code(KEYCODE_DPAD_UP, KEYMOD_ALT | KEYMOD_SHIFT, false, false),
            Some("\x1b[1;4A".to_string())
        );
        // Alt + Ctrl + ↑ (modifier 7)
        assert_eq!(
            get_code(KEYCODE_DPAD_UP, KEYMOD_ALT | KEYMOD_CTRL, false, false),
            Some("\x1b[1;7A".to_string())
        );
        // Alt + Ctrl + Shift + ↑ (modifier 8)
        assert_eq!(
            get_code(
                KEYCODE_DPAD_UP,
                KEYMOD_ALT | KEYMOD_CTRL | KEYMOD_SHIFT,
                false,
                false
            ),
            Some("\x1b[1;8A".to_string())
        );
    }
    ...
    fn test_special_keys() {
        assert_eq!(
            get_code(KEYCODE_DEL, 0, false, false),
            Some("\x7f".to_string())
        );
        assert_eq!(
            get_code(KEYCODE_DEL, KEYMOD_CTRL, false, false),
            Some("\x08".to_string())
        );
        assert_eq!(
            get_code(KEYCODE_INSERT, 0, false, false),
            Some("\x1b[2~".to_string())
        );
        assert_eq!(
            get_code(KEYCODE_FORWARD_DEL, 0, false, false),
            Some("\x1b[3~".to_string())
        );
        assert_eq!(
            get_code(KEYCODE_PAGE_UP, 0, false, false),
            Some("\x1b[5~".to_string())
        );
        assert_eq!(
            get_code(KEYCODE_PAGE_DOWN, 0, false, false),
            Some("\x1b[6~".to_string())
        );
    }
    ...
    fn test_termcap() {
        assert_eq!(
            get_code_from_termcap("k1", false, false),
            Some("\x1bOP".to_string())
        );
        assert_eq!(
            get_code_from_termcap("kd", false, false),
            Some("\x1b[B".to_string())
        );
        assert_eq!(
            get_code_from_termcap("kb", false, false),
            Some("\x7f".to_string())
        );
    }
}
