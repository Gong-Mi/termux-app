...
pub fn create_subprocess_with_data(
    ...
    if total_uid_count >= MAX_CONCURRENT_SUBPROCESSES
        || current_count >= (MAX_CONCURRENT_SUBPROCESSES - 4)
    {
        crate::utils::android_log(
            crate::utils::LogPriority::WARN,
            &format!(
                "GOVERNOR: UID PIDs: {}, Termux PIDs: {}. Throttling fork (limit {})...",
                total_uid_count, current_count, MAX_CONCURRENT_SUBPROCESSES
            ),
        );
        // 睡眠等待，给系统喘息机会
    ...
    unsafe {
        ...
        match fork() {
            ...
            Ok(ForkResult::Child) => {
                ...
                // Android owns inherited descriptor tags; Linux hosts have no fdsan ABI.
                // Keep the existing Android child policy unchanged.
                ...
                crate::utils::android_log(
                    ...
                    &format!(
                        "[PTY_TRACE] Child Process (PID={}) starting...",
                        libc::getpid()
                    ),
                );
                crate::utils::android_log(
                    crate::utils::LogPriority::INFO,
                    &format!(
                        "[PTY_TRACE] termux_data='{}' termux_prefix='{}'",
                        termux_data, termux_prefix
                    ),
                );
                ...
                crate::utils::android_log(
                    crate::utils::LogPriority::INFO,
                    &format!("[PTY_TRACE] Final ENV count: {}", final_envp.len()),
                );
                for e in &final_envp {
                ...
                if !cwd_str.is_empty() {
                    ...
                    if libc::chdir(c_cwd.as_ptr()) != 0 {
                        crate::utils::android_log(
                            ...
                            &format!("[PTY_TRACE] chdir FAILED for '{}'", cwd_str),
                        );
                    }
                }
                ...
                crate::utils::android_log(
                    crate::utils::LogPriority::INFO,
                    &format!(
                        "[PTY_CHECKPOINT] CP01: input cmd='{}' argv={:?}",
                        cmd_str, argv
                    ),
                );
                ...
                crate::utils::android_log(
                    crate::utils::LogPriority::INFO,
                    &format!(
                        "[PTY_CHECKPOINT] CP02: resolved absolute path='{}'",
                        final_cmd
                    ),
                );
                            ...
                            && buf[3] == b'F'
                        {
                            ...
                            if let Ok(shebang) = std::str::from_utf8(&buf[2..bytes_read]) {
                                ...
                                if !interpreter_line.is_empty() {
                                    let parts: Vec<&str> =
                                        interpreter_line.split_whitespace().collect();
                                    if !parts.is_empty() {
                ...
                if has_shebang && !shebang_interpreter.is_empty() {
                    ...
                    new_argv.push(
                        if !final_args.is_empty()
                            && (final_args[0].starts_with('-') || final_args[0].starts_with('/'))
                        {
                            final_args[0].clone()
                        } else {
                            final_cmd.clone()
                    ...
                    crate::utils::android_log(
                        crate::utils::LogPriority::INFO,
                        &format!(
                            "[PTY_CHECKPOINT] CP04: no ELF/shebang, fallback to sh script='{}'",
                            old_cmd
                        ),
                    );
                }
                ...
                let canonical_cmd = std::fs::canonicalize(&final_cmd)
                    .unwrap_or_else(|_| std::path::PathBuf::from(&final_cmd));
                ...
                if needs_linker {
                    ...
                    if std::path::Path::new(linker).exists() {
                        ...
                        if corrected_cmd.contains("/data/data/com.termux") {
                            corrected_cmd =
                                corrected_cmd.replace("/data/data/com.termux", &termux_data);
                        }
                        ...
                        if final_args.len() > 1 {
                            for arg in final_args.iter().skip(1) {
                                ...
                                if corrected_arg.contains("/data/data/com.termux") {
                                    corrected_arg = corrected_arg
                                        .replace("/data/data/com.termux", &termux_data);
                                }
                ...
                for arg in &final_args {
                    if let Ok(ca) = CString::new(arg.clone()) {
                        c_args.push(ca);
                    }
                }
